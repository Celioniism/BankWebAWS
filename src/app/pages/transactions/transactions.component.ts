...
import { DepositTransaction, Transactions, TransferTransaction, WithdrawTransaction } from 'src/app/routes/transactions/transactions';
...
export class TransactionsComponent implements OnInit {
  ...
  transfer: TransferTransaction = { amount: 0, reference: '', fromCardNumber: 0, toCardNumber: 0 };
  balance: number;
  userId: number;
  ...
  constructor(private _authService: AuthService, private _transactionsService: TransactionsService, private _cardService: CardsService, private router: Router, private fb: FormBuilder) { }
  ...
  submitDeposit() {
    ...
    this._transactionsService.sendDeposit(this.deposit).subscribe((response) => {
  ...
  submitTransfer() {
    ...
    console.log(this.TransferForm.value.amount, this.TransferForm.value.reference, this.TransferForm.value.tocardNumber, this.TransferForm.value.fromcardNumber)
    this._transactionsService.sendTransfer(this.transfer).subscribe((response) => {
      console.log(response);
      this.hideAll();
    });
  }
  submitWithdrawal() {
    ...
    this._transactionsService.sendWithdraw(this.withdrawal).subscribe((response) => {
    });
    this.hideAll();
  }
  redirection() {
